...
이 빠른 용어집에는 Ethereum과 관련된 많은 용어가 들어 있습니다. 이 용어는 책 전체에 걸쳐 사용되므로 빠른 참조를 위해 북마크하십시오.
...
주소, 잔고, nonce 및 선택적 저장 및 코드가 포함 된 개체입니다. 계정은 계약 계정이거나 외부 소유 계정 (EOA) 일 수 있습니다.
...
가장 일반적으로 이는 블록 체인에서 트랜잭션을 수신 (대상 주소)하거나 전송 (소스 주소) 할 수있는 EOA 또는 계약을 나타냅니다. 보다 구체적으로는 ECDSA 공개 키의 Keccak 해시 중 가장 오른쪽에있는 160 비트입니다.
